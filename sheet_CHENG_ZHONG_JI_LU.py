...
class CSheet_CHENG_ZHONG_JI_LU_Dialog(QWidget):
    def __init__(self, parent=None):
        ...
        if ok and text and not text.isspace():
            self.table_sheet_CHENG_ZHONG_JI_LU = QTableWidget()
            self.table_sheet_CHENG_ZHONG_JI_LU.setEditTriggers(QAbstractItemView.NoEditTriggers)
            self.table_sheet_CHENG_ZHONG_JI_LU.setColumnCount(17)
            self.table_sheet_CHENG_ZHONG_JI_LU.setHorizontalHeaderLabels(
                ['栏号', '出生日期', '性别', '耳号', '出生重', '第一次称重日期', '第一次称重重量', '日增量', '月增量', '第二次称重日期', '第二次称重重量', '日增量',
                 '月增量', '第三次称重日期', '第三次称重重量', '日增量', '月增量'])
            self.table_sheet_CHENG_ZHONG_JI_LU.setRowCount(1)

            self.sheet_CHENG_ZHONG_JI_LU(text)

            self.button_sheet_CHENG_ZHONG_JI_LU = QPushButton('输出表格')

            self.layout_sheet_CHENG_ZHONG_JI_LU = QVBoxLayout()
            self.layout_sheet_CHENG_ZHONG_JI_LU.addWidget(self.table_sheet_CHENG_ZHONG_JI_LU)
            self.layout_sheet_CHENG_ZHONG_JI_LU.addWidget(self.button_sheet_CHENG_ZHONG_JI_LU)

            self.setLayout(self.layout_sheet_CHENG_ZHONG_JI_LU)
    ...
    def sheet_CHENG_ZHONG_JI_LU(self, text):
        try:
            ...
            for YANG_info_item in cursor_YANG:
                ...
                current_row = self.table_sheet_CHENG_ZHONG_JI_LU.rowCount() - 1
                if YANG_info_item[1]:
                    self.table_sheet_CHENG_ZHONG_JI_LU.setItem(current_row,
                ...
                if CHAN_GAO_info_item:
                    ...
                    if CHAN_GAO_info_item[0]:
                        self.table_sheet_CHENG_ZHONG_JI_LU.setItem(current_row, 1,
                                                                   QTableWidgetItem(str(CHAN_GAO_info_item[0])))
                        self.table_sheet_CHENG_ZHONG_JI_LU.setItem(current_row, 9, QTableWidgetItem(
                            str(CHAN_GAO_info_item[0] + timedelta(days=365 / 2))))
                        self.table_sheet_CHENG_ZHONG_JI_LU.setItem(current_row, 13, QTableWidgetItem(
                            str(CHAN_GAO_info_item[0] + timedelta(days=365))))
                    if CHAN_GAO_info_item[1]:
                        self.table_sheet_CHENG_ZHONG_JI_LU.setItem(current_row,
                                                                   5, QTableWidgetItem(str(CHAN_GAO_info_item[1])))
                    if CHAN_GAO_info_item[1] and CHAN_GAO_info_item[0] and YANG_info_item[5] and YANG_info_item[4]:
                        self.table_sheet_CHENG_ZHONG_JI_LU.setItem(
                            current_row, 7,
                    ...
                    if CHAN_GAO_info_item[1] and CHAN_GAO_info_item[0] and YANG_info_item[6] and YANG_info_item[5]:
                        self.table_sheet_CHENG_ZHONG_JI_LU.setItem(
                            current_row, 11,
                    ...
                    if CHAN_GAO_info_item[0] and YANG_info_item[7] and YANG_info_item[6]:
                        self.table_sheet_CHENG_ZHONG_JI_LU.setItem(
                            current_row, 15,
                            QTableWidgetItem(
                ...
                self.table_sheet_CHENG_ZHONG_JI_LU.setRowCount(self.table_sheet_CHENG_ZHONG_JI_LU.rowCount() + 1)
    ...
    def export_sheet_CHENG_ZHONG_JI_LU(self):
        pass