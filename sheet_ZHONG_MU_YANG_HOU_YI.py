...
class CSheet_ZHONG_MU_YANG_HOU_YI_Dialog(QWidget):
    def __init__(self, parent=None):
        ...
        if ok and text and not text.isspace():
            self.table_sheet_ZHONG_MU_YANG_HOU_YI = QTableWidget()
            self.table_sheet_ZHONG_MU_YANG_HOU_YI.setEditTriggers(QAbstractItemView.NoEditTriggers)
            self.table_sheet_ZHONG_MU_YANG_HOU_YI.setColumnCount(9)
            self.table_sheet_ZHONG_MU_YANG_HOU_YI.setHorizontalHeaderLabels(
                ['与配公羊', '产羔', '活羔', '羊羔编号', '羊羔性别', '羊羔出生重', '羊羔断奶重', '羊羔六月重', '羊羔去向'])
            self.table_sheet_ZHONG_MU_YANG_HOU_YI.setRowCount(1)

            self.sheet_ZHONG_MU_YANG_HOU_YI(text)

            self.button_sheet_ZHONG_MU_YANG_HOU_YI = QPushButton('输出表格')

            self.layout_sheet_ZHONG_MU_YANG_HOU_YI = QVBoxLayout()
            self.layout_sheet_ZHONG_MU_YANG_HOU_YI.addWidget(self.table_sheet_ZHONG_MU_YANG_HOU_YI)
            self.layout_sheet_ZHONG_MU_YANG_HOU_YI.addWidget(self.button_sheet_ZHONG_MU_YANG_HOU_YI)

            self.setLayout(self.layout_sheet_ZHONG_MU_YANG_HOU_YI)
    ...
    def sheet_ZHONG_MU_YANG_HOU_YI(self, text):
        try:
            ...
            cursor_CHAN_GAO.execute(
                'select chan_gao_hao,gong_yang_hao,chan_gao,huo_gao from chan_gao where mu_yang_hao="' + text + '"')
            for CHAN_GAO_info_item in cursor_CHAN_GAO:
                print(CHAN_GAO_info_item)
                for index in range(1, 4):
                    if CHAN_GAO_info_item[index]:
                        self.table_sheet_ZHONG_MU_YANG_HOU_YI.setItem(
                            self.table_sheet_ZHONG_MU_YANG_HOU_YI.rowCount() - 1,
                            index - 1,
                ...
                if YANG_info_item is not None:
                    while YANG_info_item is not None:
                        ...
                        self.table_sheet_ZHONG_MU_YANG_HOU_YI.setRowCount(
                            self.table_sheet_ZHONG_MU_YANG_HOU_YI.rowCount() + 1)
                        YANG_info_item = cursor_YANG.fetchone()
                else:
                    self.table_sheet_ZHONG_MU_YANG_HOU_YI.setRowCount(
