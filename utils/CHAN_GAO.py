...
class CCHAN_GAO_Dialog(QDialog):
    ...
    def add_CHAN_GAO_info(self):
        try:
            ...
            if (
                self.edit_CHAN_GAO_HAO.text()
                and not self.edit_CHAN_GAO_HAO.text().isspace()
            ):
            ...
            if (
                self.edit_MU_YANG_HAO.text()
                and not self.edit_MU_YANG_HAO.text().isspace()
            ):
            ...
            if (
                self.edit_GONG_YANG_HAO.text()
                and not self.edit_GONG_YANG_HAO.text().isspace()
            ):
            ...
            if (
                self.edit_PEI_ZHONG_RI_QI_year.text()
                and not self.edit_PEI_ZHONG_RI_QI_year.text().isspace()
                and self.edit_PEI_ZHONG_RI_QI_month.text()
                and not self.edit_PEI_ZHONG_RI_QI_month.text().isspace()
                and self.edit_PEI_ZHONG_RI_QI_day.text()
                and not self.edit_PEI_ZHONG_RI_QI_day.text().isspace()
            ):
                try:
                    date_PEI_ZHONG_RI_QI = str(
                        date(
                            int(self.edit_PEI_ZHONG_RI_QI_year.text()),
            ...
            if (
                self.edit_CHAN_GAO_RI_QI_year.text()
                and not self.edit_CHAN_GAO_RI_QI_year.text().isspace()
                and self.edit_CHAN_GAO_RI_QI_month.text()
                and not self.edit_CHAN_GAO_RI_QI_month.text().isspace()
                and self.edit_CHAN_GAO_RI_QI_day.text()
                and not self.edit_CHAN_GAO_RI_QI_day.text().isspace()
            ):
                try:
                    date_CHAN_GAO_RI_QI = str(
                        date(
                            int(self.edit_CHAN_GAO_RI_QI_year.text()),
            ...
            if (
                self.edit_DUAN_NAI_RI_QI_year.text()
                and not self.edit_DUAN_NAI_RI_QI_year.text().isspace()
                and self.edit_DUAN_NAI_RI_QI_month.text()
                and not self.edit_DUAN_NAI_RI_QI_month.text().isspace()
                and self.edit_DUAN_NAI_RI_QI_day.text()
                and not self.edit_DUAN_NAI_RI_QI_day.text().isspace()
            ):
                try:
                    date_DUAN_NAI_RI_QI = str(
                        date(
                            int(self.edit_DUAN_NAI_RI_QI_year.text()),
                    ...
                    QMessageBox.information(self, "输入错误", "断奶日期输入错误。")
                    return False

            add_CHAN_GAO_index = add_CHAN_GAO_index[:-1]
            add_CHAN_GAO_values = add_CHAN_GAO_values[:-1]
            query = add_CHAN_GAO_index + add_CHAN_GAO_values + ");"
