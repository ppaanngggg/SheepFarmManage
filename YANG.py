...
class CYANG_Dialog(QDialog):
    ...
    def add_YANG_info(self):
        try:
            ...
            if (
                self.edit_XING_BIE.currentText()
                and not self.edit_XING_BIE.currentText().isspace()
            ):
            ...
            if (
                self.edit_CHU_SHENG_ZHONG.text()
                and not self.edit_CHU_SHENG_ZHONG.text().isspace()
            ):
                add_YANG_index += "chu_sheng_zhong,"
                add_YANG_values += self.edit_CHU_SHENG_ZHONG.text() + ","

            if (
                self.edit_DUAN_NAI_ZHONG.text()
                and not self.edit_DUAN_NAI_ZHONG.text().isspace()
            ):
                add_YANG_index += "duan_nai_zhong,"
                add_YANG_values += self.edit_DUAN_NAI_ZHONG.text() + ","

            if (
                self.edit_LIU_YUE_ZHONG.text()
                and not self.edit_LIU_YUE_ZHONG.text().isspace()
            ):
                add_YANG_index += "liu_yue_zhong,"
                add_YANG_values += self.edit_LIU_YUE_ZHONG.text() + ","

            if (
                self.edit_ZHOU_SUI_ZHONG.text()
                and not self.edit_ZHOU_SUI_ZHONG.text().isspace()
            ):
            ...
            query = add_YANG_index + add_YANG_values + ");"
