    ...
    def __init__(self, parent=None):
        ...
        if ok and text and not text.isspace():
            ...
            if YANG_info:
                ...
                if YANG_info[0][1]:
                    self.edit_CHAN_GAO_HAO.setText(YANG_info[0][1])
                if YANG_info[0][2]:
                    self.edit_XING_BIE.setText(YANG_info[0][2])
                if YANG_info[0][3]:
                    self.edit_ER_HAO.setText(YANG_info[0][3])
                if YANG_info[0][4]:
                    self.edit_CHU_SHENG_ZHONG.setText(str(YANG_info[0][4]))
                if YANG_info[0][5]:
                    self.edit_DUAN_NAI_ZHONG.setText(str(YANG_info[0][5]))
                if YANG_info[0][6]:
                    self.edit_LIU_YUE_ZHONG.setText(str(YANG_info[0][6]))
                if YANG_info[0][7]:
                    self.edit_ZHOU_SUI_ZHONG.setText(str(YANG_info[0][7]))
                if YANG_info[0][8]:
                    self.edit_QU_XIANG.setText(str(YANG_info[0][8]))
                if YANG_info[0][9]:
                    self.edit_CHAN_GAO_BIAN_HAO.setText(str(YANG_info[0][9]))

                self.label_edit_YANG_dialog = QLabel('\n注：(*)为必填项\n')
                self.button_edit_YANG_dialog = QPushButton('修改湖羊数据')
                self.button_edit_YANG_dialog.clicked.connect(self.button_edit_YANG_dialog_clicked)
                self.button_delete_YANG_dialog = QPushButton('删除湖羊数据')
                self.button_delete_YANG_dialog.clicked.connect(self.button_delete_YANG_dialog_clicked)
