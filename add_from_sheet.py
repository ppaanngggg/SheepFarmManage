...
def add_from_sheet(str_path, QWidget):
    ...
    for sheet in book.sheets():
        ...
        for row_index in range(3, sheet.nrows):
            mu_yang_hao = num2str(sheet.cell(row_index, 2).value)
            gong_yang_hao = num2str(sheet.cell(row_index, 3).value)
            if mu_yang_hao and gong_yang_hao and not mu_yang_hao.isspace() and not gong_yang_hao.isspace():
                try:
                    # print(sheet.name,row_index)
                    # peng_hao
                    peng_hao_tmp = num2str(sheet.cell(row_index, 0).value)
                    if peng_hao_tmp and not peng_hao_tmp.isspace():
                        peng_hao = peng_hao_tmp
                    # lan_hao
                    lan_hao_tmp = num2str(sheet.cell(row_index, 1).value)
                    if lan_hao_tmp and not lan_hao_tmp.isspace():
                        lan_hao = lan_hao_tmp
                    # tai_ci
                    tai_ci = num2str(sheet.cell(row_index, 4).value)
                    # pei_zhong_ri_qi
                    ...
                    # chan_gao
                    chan_gao = num2str(sheet.cell(row_index, 7).value)
                    # huo_gao
                    huo_gao = num2str(sheet.cell(row_index, 8).value)
                    # duan_nai_ri_qi
                    ...
                    # print(chan_gao_hao, peng_hao, lan_hao, mu_yang_hao, gong_yang_hao, tai_ci, pei_zhong_ri_qi, chan_gao_ri_qi,
                    # chan_gao, huo_gao, duan_nai_ri_qi)
                    ...
                    for row_offset in range(0, 2):
                        for col_offset in range(0, 2):
                            if row_index + row_offset < sheet.nrows:
                                bian_hao = num2str(sheet.cell(row_index + row_offset, 9 + col_offset * 3).value)
