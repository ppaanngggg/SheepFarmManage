...
class CEdit_CHAN_GAO_Dialog(CCHAN_GAO_Dialog):
        ...
        if ok and text and not text.isspace():
            ...
            if CHAN_GAO_info:
                ...
                if CHAN_GAO_info[0][6]:
                    self.edit_PEI_ZHONG_RI_QI_year.setText(
                        str(CHAN_GAO_info[0][6].year)
                    )
                    self.edit_PEI_ZHONG_RI_QI_month.setText(
                        str(CHAN_GAO_info[0][6].month)
                    )
                ...
                if CHAN_GAO_info[0][7]:
                    ...
                    self.edit_CHAN_GAO_RI_QI_month.setText(
                        str(CHAN_GAO_info[0][7].month)
                    )
                ...
                if CHAN_GAO_info[0][10]:
                    self.edit_DUAN_NAI_RI_QI_year.setText(
                        str(CHAN_GAO_info[0][10].year)
                    )
                    self.edit_DUAN_NAI_RI_QI_month.setText(
                        str(CHAN_GAO_info[0][10].month)
                    )
                ...
                self.button_edit_CHAN_GAO_dialog.clicked.connect(
                    self.button_edit_CHAN_GAO_dialog_clicked
                )
                self.button_delete_CHAN_GAO_dialog = QPushButton("删除产羔信息")
                self.button_delete_CHAN_GAO_dialog.clicked.connect(
                    self.button_delete_CHAN_GAO_dialog_clicked
                )
    ...
    def button_edit_CHAN_GAO_dialog_clicked(self):
        if (
            self.delete_CHAN_GAO_info(self.edit_CHAN_GAO_HAO.text())
            and self.add_CHAN_GAO_info()
        ):
